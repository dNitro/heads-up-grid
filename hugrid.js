...
(function($) {
  ...
  $.fn.HUG = function(options) {
    ...
    function makeHUGrid() {
      ...
      if (typeof settings.pageLeftMargin === "number") {
        ...
        if (settings.pageUnits === "%") {
          ...
          $("#hugrid").css(
            "margin-left",
            "-" + settings.pageWidth / 2 + settings.pageUnits
          );
        }
      }
      ...
      $("#hugridRows div.hugrow").css(
        "margin-top",
        settings.rowHeight - 1 + "px"
      );
    ...
    function setGridOnLoad() {
      if (
        typeof settings.gridOnLoad === "undefined" ||
        settings.gridOnLoad === "off"
      ) {
        $("#hugridButton").toggleClass("buttonisoff");
        $("#hugrid").toggle();
        $("#hugridRows").toggle();
